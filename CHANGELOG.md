...
- **`check-schema-drift.js` perde índices em silêncio quando um `COMMENT` contém `;`.** Ele delimita o corpo de um `ALTER TABLE` no primeiro `;` do texto, sem saber que strings existem — então um ponto e vírgula dentro de `COMMENT '...'` corta a instrução e as cláusulas `ADD INDEX` seguintes somem da declaração esperada, com o comando saindo em código 0. Descoberto ao escrever a v15: três índices ficaram invisíveis. Contornado removendo os `;` dos comentários da migration e registrado no cabeçalho dela — **o regex do checador continua com o defeito** e merece correção separada.
