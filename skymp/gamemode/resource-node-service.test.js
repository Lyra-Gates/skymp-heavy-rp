...
function makeHarness(options = {}) {
  ...
  const conn = {
    ...
    query: async (sql, params = []) => {
      ...
      if (/SELECT transaction_id FROM inventory_transactions WHERE idempotency_key = \?/i.test(sql)) {
        const found = state.ledger.find((entry) => entry.idempotencyKey === params[0]);
        return [found ? [{ transaction_id: 'existing' }] : []];
      }
      if (/SELECT last_gathered_at FROM resource_node_gathers/i.test(sql)) {
        const value = state.gathers.get(`${params[0]}:${params[1]}`);
        return [value ? [{ last_gathered_at: value }] : []];
      }
      if (/INSERT INTO resource_node_gathers/i.test(sql)) {
        state.gathers.set(`${params[0]}:${params[1]}`, params[2]);
  ...
  const db = {
    ...
    query: async (sql, params = []) => {
      ...
      if (/SELECT form_desc FROM resource_nodes WHERE enabled = 1 ORDER BY id/i.test(sql)) {
        return state.nodes.filter((row) => row.enabled).map((row) => ({ form_desc: row.form_desc }));
      }
...
describe('resource-node-service — descoberta física', () => {
  it('lista somente FormDesc válidos de nós habilitados', async () => {
    const h = makeHarness({ nodes: [
      baseNode(),
      baseNode({ id: 2, form_desc: '4a2f1:Skyrim.esm', enabled: 0 }),
      baseNode({ id: 3, form_desc: 'invalido', enabled: 1 })
    ] });
    assert.deepStrictEqual(await resourceNodeService.listEnabledNodes(h.dependencies), [FORM_DESC]);
  });
});
...
describe('resource-node-service — consume', () => {
  it('decrementa o nó e entrega o item na MESMA transação', async () => {
    ...
    const r = await resourceNodeService.consume({ characterId: CHAR_A, actorId: 0xff01, formDesc: FORM_DESC, requestId: 'request-0001' }, h.dependencies);
  ...
  it('recusa nó esgotado, sem entregar item nem alterar o inventário', async () => {
    ...
    const r = await resourceNodeService.consume({ characterId: CHAR_A, formDesc: FORM_DESC, requestId: 'request-0001' }, h.dependencies);
  ...
  it('recusa characterId inválido sem tocar o banco', async () => {
    ...
    const r = await resourceNodeService.consume({ characterId: -1, formDesc: FORM_DESC, requestId: 'request-0001' }, h.dependencies);
    assert.deepStrictEqual(r, { ok: false, code: 'invalid_character' });
    assert.deepStrictEqual(h.state.events, []);
  });

  it('exige requestId para defesa ponta a ponta contra replay', async () => {
    const h = makeHarness({ nodes: [baseNode()] });
    const r = await resourceNodeService.consume({ characterId: CHAR_A, formDesc: FORM_DESC }, h.dependencies);
    assert.deepStrictEqual(r, { ok: false, code: 'invalid_request_id' });
    assert.strictEqual(h.state.events.length, 0);
  });

  it('o mesmo requestId não consome nem recompensa duas vezes', async () => {
    const h = makeHarness({ nodes: [baseNode()] });
    const opts = { characterId: CHAR_A, formDesc: FORM_DESC, requestId: 'request-replay-01' };
    const first = await resourceNodeService.consume(opts, h.dependencies);
    const capacityAfterFirst = h.state.nodes[0].current_capacity;
    const inventoryAfterFirst = h.state.inventory[`${CHAR_A}:${RESOURCE_ID}`];
    const second = await resourceNodeService.consume(opts, h.dependencies);
    assert.strictEqual(first.ok, true);
    assert.deepStrictEqual(second, { ok: true, replay: true, data: { formDesc: FORM_DESC } });
    assert.strictEqual(h.state.nodes[0].current_capacity, capacityAfterFirst);
    assert.strictEqual(h.state.inventory[`${CHAR_A}:${RESOURCE_ID}`], inventoryAfterFirst);
  });

  it('cooldown persiste por personagem e nó e recusa request diferente imediato', async () => {
    const h = makeHarness({ nodes: [baseNode()] });
    const first = await resourceNodeService.consume({
      characterId: CHAR_A, formDesc: FORM_DESC, requestId: 'request-cooldown-01', cooldownMs: 30000
    }, h.dependencies);
    const second = await resourceNodeService.consume({
      characterId: CHAR_A, formDesc: FORM_DESC, requestId: 'request-cooldown-02', cooldownMs: 30000
    }, h.dependencies);
    assert.strictEqual(first.ok, true);
    assert.strictEqual(second.ok, false);
    assert.strictEqual(second.code, 'cooldown');
    assert.ok(second.data.retryAfterMs > 0 && second.data.retryAfterMs <= 30000);
  });
  ...
  it('travou a linha do nó por FOR UPDATE antes de decidir (§25 do briefing — duas colheitas, um veio)', async () => {
    const h = makeHarness({ nodes: [baseNode()] });
    await resourceNodeService.consume({ characterId: CHAR_A, formDesc: FORM_DESC, requestId: 'request-0001' }, h.dependencies);
  ...
  it('esvazia exatamente no limite: última unidade some, próxima recusa', async () => {
    ...
    const primeira = await resourceNodeService.consume({ characterId: CHAR_A, formDesc: FORM_DESC, requestId: 'request-0001' }, h.dependencies);
    assert.strictEqual(primeira.ok, true);
    assert.strictEqual(primeira.data.capacity, 0);

    const segunda = await resourceNodeService.consume({ characterId: CHAR_A, formDesc: FORM_DESC, requestId: 'request-0002' }, h.dependencies);
  ...
  it('falha de infraestrutura LANÇA e faz rollback, nunca entrega item parcial', async () => {
    ...
    await assert.rejects(() => resourceNodeService.consume({ characterId: CHAR_A, formDesc: FORM_DESC, requestId: 'request-0001' }, h.dependencies));
    assert.ok(h.state.events.includes('rollback'));
    assert.strictEqual(h.state.inventory[`${CHAR_A}:${RESOURCE_ID}`], undefined);
  });
});
...
describe('a migration-v27 declara toda coluna que resource-node-service usa', () => {
  const sql = fs.readFileSync(
    path.resolve(__dirname, '..', 'packages', 'database', 'migration-v27-resource-nodes.sql'),
    'utf8'
  );
  ...
  for (const coluna of COLUNAS_USADAS) {
    it(`declara a coluna '${coluna}'`, () => {
      assert.ok(criacao.includes(`\`${coluna}\``), `coluna '${coluna}' usada em resource-node-service.js mas ausente da migration-v27`);
    });
  }
  ...
  it('declara cooldown persistente por nó e personagem', () => {
    assert.ok(sql.includes('CREATE TABLE IF NOT EXISTS `resource_node_gathers`'));
    assert.ok(sql.includes('PRIMARY KEY (`resource_node_id`, `character_id`)'));
  });
});
