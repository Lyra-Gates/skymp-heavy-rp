...
describe('crafting-service — consumo e entrega numa transação só', () => {
  ...
  describe('gate de profissão (migration-v23)', () => {
    beforeEach(() => {
...
describe('a migration-v23 declara toda coluna que o gate de profissão do crafting usa', () => {
  const sql = fs.readFileSync(
    path.resolve(__dirname, '..', 'packages', 'database', 'migration-v23-crafting-profession-gate.sql'),
    'utf8'
  );
  ...
  for (const coluna of ['required_profession', 'required_rank']) {
    it(`declara a coluna '${coluna}'`, () => {
      assert.ok(alteracao.includes(`\`${coluna}\``), `coluna '${coluna}' usada em crafting-service.js mas ausente da migration-v23`);
    });
  }
});
