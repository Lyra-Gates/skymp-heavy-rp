...
const serverOptionsMock = {
  get: (key) => {
    ...
    if (key === 'mining.cooldownMs') return cooldownMs;
    return undefined;
  }
};
...
function executeMining(service) {
  requestSequence += 1;
  return service.execute(ATOR, {
    action: 'mining.mine',
    targetId: VEIO_FORMID,
...
describe('mining-service — mining.mine no Interaction Framework', () => {
  ...
  it('longe do veio, execute recusa no estágio de distância e não consome', async () => {
    ...
    const service = montarServico();
    const r = await executeMining(service);
