...
- **Launcher App**: Launcher em React + Electron (Vite) em `apps/launcher`, controlando autenticação, paridade, configuração e boot da build. Além das correções de configuração/OAuth da auditoria de agosto, o bootstrap agora falha fechado: valida e relê os dois arquivos do cliente, remove identidade legada, usa o destino direto e só anuncia sucesso após o Windows confirmar o processo. O build empacotado foi validado em 26/08/2026; o handshake com dois clientes reais continua pendente. Ver [ADR-012](docs/technical/ADR_012_LAUNCHER_CONNECTION_BOOTSTRAP.md) e [QA_REPORT_2026-08.md](docs/technical/QA_REPORT_2026-08.md) 2.1 e 2.2.
...
- **Crime & Proveniência (21-22/08/2026)**: roubo passa a deixar rastro. `item_instances` (`ENABLE_CRIME_SYSTEM`) dá a cada item roubado um UUID com histórico de posse — um item recém-roubado fica `hot` por uma janela configurável e é restituído automaticamente se o ladrão desconectar nela (anti-combat-log). Revista institucional da guarda revela o dono original de um item roubado ("este anel pertence a Balgruuf"), nunca um veredito do servidor. Confisco passa a linkar com a instância rastreada. Ver [CRIME_SYSTEM_AUDIT.md](docs/technical/CRIME_SYSTEM_AUDIT.md).

- **Gate de Profissão no Crafting + Assinatura do Artesão (22/08/2026)**: `crafting-service.js` (`ENABLE_CRAFTING_SERVICE`) passa a checar `required_profession`/`required_rank` por receita antes de craftar, e concede XP de profissão por craft. Em cima disso, um artesão com rank suficiente pode assinar o que craft — `/craft <recipeId> [estacao] [dedicatoria]` — e a revista institucional da guarda mostra essa autoria, aberto a qualquer profissão de crafting (Fundidor, Ferreiro, Curtidor, Encantador, Cozinheiro). Nenhuma receita real de Ferreiro/Encantador/Cozinheiro existe ainda — falta um `result_base_id` de arma/armadura confirmado, nunca inventado. Ver [CRAFTING_SYSTEM.md](docs/gameplay/CRAFTING_SYSTEM.md) e [MAKERS_MARK.md](docs/design/MAKERS_MARK.md).

> ⚠️ **O servidor ainda não foi validado com jogadores reais e permanece NO-GO.** AUTH-01 foi corrigido em 24/08/2026: o launcher agora grava `gameData.session`, não grava o `profileId` controlado pelo cliente no fluxo online, e `offlineMode: false` permite ao SkyMP resolver a identidade pelo Master API. Na conferência local de 26/08/2026, o gamemode possui **1.262 testes (1.261 aprovados e 1 falha conhecida)**, migrations até **v29**, **80 tabelas declaradas** e typecheck limpo. MariaDB, rotas reais de Public Work e validação in-game continuam pendentes. O total agregado de testes de produto de 24/08 não foi recalculado. Acompanhe o [plano operacional](docs/roadmap/PRODUCTION_READINESS_ACTION_PLAN.md) e o [parecer NO-GO](docs/operations/PRODUCTION_BLOCKERS_2026-08-24.md).
