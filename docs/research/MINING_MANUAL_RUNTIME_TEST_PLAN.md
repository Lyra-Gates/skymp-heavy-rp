...
> **Atualização de 24/08/2026:** `mining.mine` exige `requestId`, possui
> deduplicação ponta a ponta e cooldown persistente. Nos cenários, envie um ID
> de 8–48 caracteres; repetir o mesmo ID deve produzir uma única linha de
> ledger, enquanto um ID diferente dentro do cooldown deve ser recusado.
> Com `ENABLE_INTERACTION_PROMPT=true`, o nó também deve aparecer pelo prompt
> `[E]` por proximidade; o uso do DevTools ficou apenas como diagnóstico.
