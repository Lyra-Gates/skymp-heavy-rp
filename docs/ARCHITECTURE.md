...
- **Tabelas Principais:** `accounts`, `characters`, `character_inventory`, `audit_logs`, `whitelist_applications`, `staff_roles`, `factions`, `holds`, `properties`, `market_stalls`, `crafting_recipes`, `crafting_ingredients`. O schema completo está em `skymp/packages/database/schema.sql` mais todas as migrations disponíveis (`v2`–`v28` atualmente), aplicadas **em ordem numérica**. O inventário autoritativo é produzido por `npm run check:schema:list`.
...
- Fornece ao **Launcher** a troca de OAuth do Discord (`POST /api/launcher/oauth/exchange`, que também emite o ticket de lançamento e uma sessão de launcher de 30 dias) e o recebimento de crash reports. `POST /api/launcher/session/refresh-ticket` troca a sessão por um ticket de lançamento novo sem repetir o OAuth — corrige o caso em que a fila admite direto (sem `pollTicket` de reposição) e uma segunda tentativa de jogar reenviava o ticket já consumido (`migration-v25-launcher-sessions.sql`, `LAUNCHER_DISTRIBUTION.md` §4). O manifesto de mods **não** vem daqui — vem do `apps/game-api` e do GitHub Releases (ver 1.3.1 e `LAUNCHER_DISTRIBUTION.md`).
...
⚠️ **Nunca rodou numa sessão real.** Na conferência local de 26/08/2026, a suíte inteira do gamemode tinha **1.262 testes: 1.261 aprovados e 1 falha conhecida em Safe Zones**. Essa cobertura automatizada não substitui clientes reais: a CEF foi reescrita para `interaction:*` e passa em verificação estática, mas ainda não foi exercitada dentro de um CEF real.
...
- **Bootstrap de conexão fail-closed**: depois da fila, o launcher valida e grava os dois contratos do cliente (`skymp_config.json` e `skymp5-client-settings.txt`), remove identidade legada controlada pelo cliente e usa `server-info-ignore:true` para conectar diretamente ao host/porta configurados. O SKSE só é criado depois da releitura dos arquivos; `spawn` sem shell precisa confirmar evento e PID antes de a UI mostrar sucesso. Motivos e consequências: [ADR-012](technical/ADR_012_LAUNCHER_CONNECTION_BOOTSTRAP.md).
