...
- **Main tables:** `accounts`, `characters`, `character_inventory`, `audit_logs`, `whitelist_applications`, `staff_roles`, `factions`, `holds`, `properties`, `market_stalls`, `crafting_recipes`, `crafting_ingredients`. The full schema lives in `skymp/packages/database/schema.sql` plus every available migration (currently `v2`–`v26`), applied **in numeric order**. `npm run check:schema:list` produces the authoritative inventory.
