...
- **Main tables:** `accounts`, `characters`, `character_inventory`, `audit_logs`, `whitelist_applications`, `staff_roles`, `factions`, `holds`, `properties`, `market_stalls`, `crafting_recipes`, `crafting_ingredients`. The full schema lives in `skymp/packages/database/schema.sql` plus every available migration (currently `v2`–`v25`), applied **in numeric order**. `npm run check:schema:list` produces the authoritative inventory.
...
- Serves the **launcher** the Discord OAuth exchange (`POST /api/launcher/oauth/exchange`, which also issues the launch ticket and a 30-day launcher session) and receives crash reports. `POST /api/launcher/session/refresh-ticket` exchanges the session for a fresh launch ticket without repeating the OAuth flow — fixes the case where the queue admits immediately (no replacement `pollTicket`) and a second play attempt resent the already-consumed ticket (`migration-v25-launcher-sessions.sql`, `LAUNCHER_DISTRIBUTION.md` §4). The mods manifest does **not** come from here — it comes from `apps/game-api` and GitHub Releases (see 1.3.1 and `LAUNCHER_DISTRIBUTION.md`).
